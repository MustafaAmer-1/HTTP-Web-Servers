...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("One Or more fields is missing Check them out".encode())
            return
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Bad Long URI, Cannot feth \" {} \" ,Please write a valid one".format(longuri).encode())

class ThreadHTTPServer(ThreadingMixIn, http.server.HTTPServer):
    "This is an HTTPServer that supports thread-based concurrency."

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8000))   # Use PORT if it's there in environ.
    server_address = ('', port)
